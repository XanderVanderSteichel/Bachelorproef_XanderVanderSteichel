...
\subsection{Doel}
In deze fase werd de Jenkins-oplossing onderworpen aan functionele en foutgebaseerde testscenario’s om de betrouwbaarheid, foutafhandeling en prestaties te evalueren.

\subsection{Testscenario’s}
\begin{itemize}
    \item Wat gebeurt er als een G-code bestand niet gevonden wordt?
    \item Hoe wordt gereageerd op printfouten of onderbrekingen?
    \item Wat gebeurt er bij stroomuitval van de printer?
    \item Hoe gaat het systeem om met meerdere bestellingen tegelijk?
\end{itemize}

\subsection{Resultaten}
\begin{itemize}
    \item Curl-aanroepen bleken betrouwbaarder dan externe Python-libraries zoals \texttt{moonrakerpy}, en zorgden voor snelle debugging.
    \item Jenkins-pipeline werkte stabiel bij normale belasting en kon foutmeldingen correct opvangen (bijvoorbeeld bij ontbrekende bestanden).
    \item Parallelle bestellingen werden voorlopig nog niet ondersteund, maar dit wordt meegenomen naar optimalisatiefase.
    \item Printproces kon correct gestart en gestopt worden via API-aanroepen.
\end{itemize}

\subsection{Afsluitende Opmerkingen}
\begin{itemize}
    \item Niet alle packages uit \texttt{requirements.txt} werden uiteindelijk gebruikt.
    \item Voor productie is het aanbevolen om Jenkins permanent lokaal te draaien op een Raspberry Pi in plaats van via Ngrok.
    \item Curl-aanpak zal in Fase 4 verder worden geoptimaliseerd en mogelijk vervangen door Python-scripts indien dit voordelen biedt.
\end{itemize}

\subsection{Besluit Fase 3}
Op basis van de testresultaten blijft Jenkins de meest geschikte CI/CD-tool voor het project. De pipeline zal verder verfijnd worden in Fase 4, met aandacht voor fouttolerantie, performantie en onderhoudsgemak.
